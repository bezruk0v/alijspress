...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Генерация карточек товаров в каталоге
  ...
  // Рендеры карточек товаров в каталоге и корзине
  const renderCard = (goods) => {
    ...
    if (goods.length) {
        ...
        goodsWrapper.textContent = ' Х Извините, мы не нашли товаров по вашему запросу ';
    }
  };
...
// События
  ...
  const chooseCategory = (event) => {
    ...
    if (target.classList.contains('category-item')) {
      ...
      getGoods(renderCart, goods => goods.filter((item) => item.category.includes(cat)));
    }
  };
  ...
  // Хэндлеры - определяем куда и что кликнули
